...
// public methods
MeetAndroid::MeetAndroid():softSerial(2,3)
{
...
MeetAndroid::MeetAndroid(SoftSerial &sSerial):softSerial(sSerial)
{
    // it is hard to use member function pointer together with normal function pointers.
    customErrorFunc = false;
	errorFunc = 0;
...
MeetAndroid::MeetAndroid(H_voidFuncPtr err):softSerial(2,3)
{
...
bool MeetAndroid::receive(){
	...
	while(!timeout)
	{
		...
		if(softSerial.available() <= 0 && !timeout){
			if(waitTime > 0) delayMicroseconds(waitTime);
			if(softSerial.available() <= 0) timeout = true;
		}
	}
	return timeout;
}
...
void MeetAndroid::send(char c ){
	softSerial.print(startFlag);
	softSerial.print(c);
	softSerial.print(ack);
}
